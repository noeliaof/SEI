...
grid.arrange(plt_gam, plt_lnorm, plt_weib, nrow = 1)
@
\caption{Distribution of standardised wind speed indices constructed using the gamma, log-normal, and Weibull distributions. The standard normal density is displayed in blue.}

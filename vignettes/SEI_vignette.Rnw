...
lines(seq(0, 10, 0.01), dnorm(seq(0, 10, 0.01), out_norm$params[1], out_gamma$params[2]), col = "blue")
...
plot_raw <- plot_sei(de_wind_d, type = "hist",
                     lab = "Wind speed (m/s)")
plot_ind <- plot_sei(sei_ws_wd, type = "hist", lab = "SEI")
grid.arrange(plot_raw, plot_ind, nrow = 1)
@
...
To conclude this section, wind droughts are calculated using the \code{get_drought()} function. The phenomenon of wind droughts has gained increasing attention in recent years \citep[e.g.,][]{Otero2022}, and we anticipate that future studies in this field can greatly benefit from the use of the \pkg{SEI} package.

...
Within the same context of renewable energy, the standardised indices can be applied to assess the wind resource, a variable of interest directly related to wind power production. Hourly estimates of near-surface wind speed of 10 m (WS) are publicly available at\citep[see][for details]{HersbachEtAl2023}. Similarly than in \citep{OteroEtAl2022b}, daily time series of WS corresponding to the period 1979-2019 were obtained from the original data, which was subsequently country-wise aggregated.Consistent with the energy example, the use case is restricted to Germany. For the sake of clarity in the visualization, the analysis is limited to 3 years (2017-2019) . The subset can be accessed from \pkg{SEI} using

<<load_wind_data>>=
data("data_wind_de", package = "SEI")
data_wind_de <- subset(data_wind_de, format(date,"%Y")>=2017 & format(date,"%Y")<=2019)
head(data_wind_de)
@

As in the previous case, the data can be rescaled from daily to weekly and to monthly time series using \pkg{xts} functionality.

<<rescale_wind>>=
de_wind_d <- xts::xts(data_wind_de$wsmean, data_wind_de$date) # convert to xts
de_wind_w <- xts::apply.weekly(de_wind_d, "mean")   # weekly data
de_wind_m <- xts::apply.monthly(de_wind_d, "mean")  # monthly data
@

A preliminary examination of the wind speed is obtained by using the \code{plot_sei()} function. Figure \ref{fig:ts_ws_raw} depicts consistent behavior across all three time series displays, with the monthly time series exhibiting the smoothest patterns, as anticipated.

\begin{figure}
<<plot_raw_ts_wind, dev='pdf', fig.width=9, fig.height=3, fig.align="center", out.width = "\\linewidth">>=
lab <- "10m Wind speed (m/s)"
plot_ws_d <- plot_sei(de_wind_d, lab = lab, title = "Daily")
plot_ws_w <- plot_sei(de_wind_w, lab = lab, title = "Weekly")
plot_ws_m <- plot_sei(de_wind_m, lab = lab, title = "Monthly")
grid.arrange(plot_ws_d, plot_ws_w, plot_ws_m, nrow = 1)
@
\caption{Daily, weekly and monthly time series averages of 10m WS in Germany during the period 1979-2019.}
\label{fig:ts_ws_raw}
\end{figure}


The raw data can be transformed into a standardised index using \code{std_index()}. Prior to that, one can test some of the distributions available in the \pkg{SEI} package as shown below

<<calculate_distribution, warning=FALSE>>=
# Set up the layout for a 1x2 grid
par(mfrow = c(1, 3))
# Plot 1: Normal distribution
out_norm <- fit_dist(data_wind_de$wsmean, dist = "norm")
hist(data_wind_de$wsmean, breaks = 30, probability = TRUE, main = "Normal Distribution", xlab = "Wind Speed", ylab = "Probability Density")
lines(seq(0, 10, 0.01), dnorm(seq(0, 10, 0.01), out_norm$params[1], out_norm$params[2]), col = "blue")

# Plot 1: Gamma distribution
out_gamma <- fit_dist(data_wind_de$wsmean, dist = "gamma")
hist(data_wind_de$wsmean, breaks = 30, probability = TRUE, main = "Gamma Distribution", xlab = "Wind Speed", ylab = "Probability Density")
lines(seq(0, 10, 0.01), dgamma(seq(0, 10, 0.01), out_gamma$params[1], out_gamma$params[2]), col = "blue")

# Plot 2: Weibull distribution
out_weibull <- fit_dist(data_wind_de$wsmean, dist = "weibull")
hist(data_wind_de$wsmean, breaks = 30, probability = TRUE, main = "Weibull Distribution", xlab = "Wind Speed", ylab = "Probability Density")
lines(seq(0, 10, 0.01), dweibull(seq(0, 10, 0.01), out_weibull$params[1], out_weibull$params[2]), col = "blue")
...
Similarly than in the previous application, the functionality of the \pkg{SEI} package can be further explore the wind droughts chatacteristics, after obtaining the standardised index. As described below the \code{std_index()} function returns the index, which can follow an specific distribution. Figure \ref{fig:ts_sei_ws} shows the values of SEI for wind speed at different time scales, namely, daily, weekly and monthly and figure \ref{fig:dist_wind} illustrates the daily wind speed conditions and the corresponding SEI values for Germany.

<<use_rescale_wind>>=
sei_ws_d <- std_index(de_wind_d)
@


\begin{figure}
<<plot_sei_ts_wind, dev='pdf', fig.width=9, fig.height=3, fig.align="center", out.width = "\\linewidth">>=
lab <- "SEI 10m-WS"
plot_h <- plot_sei(std_index(de_wind_d), lab = lab, title = "Daily")
plot_d <- plot_sei(std_index(de_wind_d, timescale = "days", rescale = "weeks"), lab = lab, title = "Weekly")
plot_w <- plot_sei(std_index(de_wind_d, timescale = "days", rescale = "months"), lab = lab, title = "Monthly")
grid.arrange(plot_h, plot_d, plot_w, nrow = 1)
@
\caption{Time series of SEI values in Germany at daily, weekly and monthly time scales during the period 2017-2019.}
\label{fig:ts_sei_ws}
\end{figure}


\begin{figure}
<<plot_dist_wind, dev='pdf', fig.width=9, fig.height=3, fig.align="center", out.width = "\\linewidth">>=
plot_raw <- plot_sei(de_wind_d, type = "hist",
                     lab = "Wind speed (m/s)")
plot_ind <- plot_sei(sei_ws_d, type = "hist", lab = "SEI")
grid.arrange(plot_raw, plot_ind, nrow = 1)
@

\caption{Histogram of daily 10m wind speed and the corresponding SEI values in Germany during the period 2017-2019.}
\label{fig:dist_wind}
\end{figure}

To conclude this section, wind droughts are calculated using the \code{get_drought()} function. The phenomenon of wind droughts has gained increasing attention in recent years \citep[e.g.,][]{OteroEtAl2022b}, and we anticipate that future studies in this field can greatly benefit from the use of the \pkg{SEI} package.


<<define_droughts_wind>>=
thresholds <- -qnorm(c(0.9, 0.95, 0.975)) # -1.28, -1.64, -1.96
drought_df <- get_drought(sei_ws_d, thresholds, exceed = F)
@

We can now further explore the wind drought characteristics, such as, the frequency, duration or intensity as follows

<<event_frequency_wind>>=
num_ev <- sapply(0:3, function(i) sum(drought_df$ins == i))
names(num_ev) <- c("None", "Moderate", "Severe", "Extreme")
print(num_ev)
@

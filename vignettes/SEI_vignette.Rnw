...
%We can easily generate uniformly distributed PIT values for censored variables using appropriate randomisation, but this would result in different instances of zero precipitation corresponding to different standardised index values, for example, which is likely not the ideal solution in practice.
...
\end{cases}
\]
Note that for $X > 0$, $F(X)$ can also be written as $F(X) = F(a) + (1 - F(a)) F(X \mid X > a)$, which is equivalent to the expression given in \citet[Equation 4]{StaggeEtAl2015}.
...
\]
Since $F(X) \mid X > a$ is uniformly distributed between $\mathsf{P}(X = a)$ and 1, the first expectation is equal to $(\mathsf{P}(X = a) + 1)/2$. Rearranging gives that $c = \mathsf{P}(X = a)/2$ yields $\mathsf{E}[\tilde{F(X)}] = 1/2$. Hence, probability indices at the censored values should be replaced with $c = \mathsf{P}(X = a)/2$.
...
The constant proposed by \citet[Equation 3]{StaggeEtAl2015} is a sample estimate of $\mathsf{P}(X = a)/2$. However, since the Gaussian inverse function $\Phi^{-1}$ is a non-linear function, generally $\mathsf{E}[\Phi^{-1}(Z)] \neq \Phi^{-1}(\mathsf{E}[Z])$, for any arbitrary random variable $Z$. Hence, normal standardised indices will generally not have expectation zero when the constant $c$ is chosen as above; this is also highlighted by Figure 1b in \cite{StaggeEtAl2015}, where the mean of the normal standardised indices with their proposed censoring methodology is generally not zero.
